    ...
    for (let i = 0; i < json.length; i++) {
      ...
      for (let j = 0; j < template.exercises.length; j++) {
        ...
        if (!validMuscleGroups.includes(exercise.muscleGroup)) {
          errors.push(`Template ${i + 1}, Exercício ${j + 1}: grupo muscular inválido: ${exercise.muscleGroup}`)
          continue
        }
        ...
        if (typeof exercise.reps !== 'number' || exercise.reps <= 0) {
          errors.push(`Template ${i + 1}, Exercício ${j + 1}: repetições deve ser um número maior que 0`)
          continue
        }

        if (typeof exercise.load !== 'number' || exercise.load < 0) {
          errors.push(`Template ${i + 1}, Exercício ${j + 1}: carga deve ser um número maior ou igual a 0`)
          continue
        }

        if (typeof exercise.restSec !== 'number' || exercise.restSec < 0) {
          errors.push(`Template ${i + 1}, Exercício ${j + 1}: descanso deve ser um número maior ou igual a 0`)
          continue
        }
